...
export function activate(context: vscode.ExtensionContext) {
	...
	const diagnosticCollection =
		vscode.languages.createDiagnosticCollection("alloy");
	...
	context.subscriptions.push(
		vscode.workspace.onDidChangeTextDocument((event) =>
			validateAlloyDocument(event.document, diagnosticCollection),
		),
	);
	context.subscriptions.push(
		...
		vscode.commands.registerCommand("alloy.generateDocumentation", () => {
			...
			vscode.workspace
				...
				.then(
					...
					(error) => {
						console.error(
							"Failed to open or display generated documentation.",
							error,
						);
						const message =
							error instanceof Error ? error.message : String(error);
						vscode.window.showErrorMessage(
							`Failed to open generated documentation: ${message}`,
						);
					},
				);
		}),
 ...
 */
function validateAlloyDocument(
	document: vscode.TextDocument,
 ...
 */
function validatePostgresBlock(
	lines: string[],
	startLine: number,
...
function getComponentDescription(componentName: string): string {
	const descriptions: { [key: string]: string } = {
		...
		"prometheus.exporter.postgres":
			"Exports PostgreSQL database metrics for Prometheus",
